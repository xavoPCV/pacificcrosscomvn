...
(function(c,a){a.extend(a,{DOM:{resetTranslate:function(b){a.browser.webkit?a.browser.is3dSupported?jQuery(b).css({webkitTransform:"translate3d(0px, 0px, 0px)"}):jQuery(b).css({webkitTransform:"translate(0px, 0px)"}):jQuery(b).css({webkitTransform:"translate(0px, 0px)",MozTransform:"translate(0px, 0px)",transform:"translate(0px, 0px)"})},createElement:function(a,d,e){a=jQuery("<"+a+"></"+a+">");a.attr(d);a.append(e);return a[0]},appendChild:function(a,d){jQuery(d).append(a)},appendText:function(a,d){jQuery(d).text(a)},appendToBody:function(a){jQuery("body").append(a)},
removeChild:function(a){jQuery(a).empty().remove()},removeChildren:function(a){jQuery(a).empty()},hasAttribute:function(b,d){return a.isNothing(jQuery(b).attr(d))},getAttribute:function(a,d){return jQuery(a).attr(d)},setAttribute:function(a,d,e){jQuery(a).attr(d,e)},removeAttribute:function(a,d){jQuery(a).removeAttr(d)},addClass:function(a,d){jQuery(a).addClass(d)},removeClass:function(a,d){jQuery(a).removeClass(d)},hasClass:function(a,d){jQuery(a).hasClass(d)},setStyle:function(b,d,e){a.isObject(d)?jQuery(b).css(d):jQuery(b).css(d,e)},getStyle:function(a,
d){return jQuery(a).css(d)},hide:function(a){jQuery(a).hide()},show:function(a){jQuery(a).show()},width:function(b,d){a.isNothing(d)||jQuery(b).width(d);return jQuery(b).width()},outerWidth:function(a){return jQuery(a).outerWidth()},height:function(b,d){a.isNothing(d)||jQuery(b).height(d);return jQuery(b).height()},outerHeight:function(a){return jQuery(a).outerHeight()},documentWidth:function(){return jQuery(document.documentElement).width()},documentHeight:function(){return jQuery(document.documentElement).height()},bodyWidth:function(){return jQuery(document.body).width()},
bodyHeight:function(){return jQuery(document.body).height()},windowWidth:function(){if(!c.innerWidth)return jQuery(c).width();return c.innerWidth},windowHeight:function(){if(!c.innerHeight)return jQuery(c).height();return c.innerHeight},windowScrollLeft:function(){if(!c.pageXOffset)return jQuery(c).scrollLeft();return c.pageXOffset},windowScrollTop:function(){if(!c.pageYOffset)return jQuery(c).scrollTop();return c.pageYOffset},addEventListener:function(a,d,e){jQuery(a).bind(d,e)},removeEventListener:function(a,d,e){jQuery(a).unbind(d,
...
(function(c,a){a.extend(a,{Animation:{stopFade:function(a){jQuery(a).stop(!0,!0)},fadeIn:function(b,d,e,c){d=a.coalesce(d,1);e=a.coalesce(e,500);jQuery(b).fadeTo(e,d,c)},fadeOut:function(b,d,e){a.isNothing(d)&&(d=500);jQuery(b).fadeTo(d,0,e)},slideBy:function(b,d,e,c,f){a.isNothing(c)&&(c=500);d=jQuery.fn.translation?{left:"+="+d+"px",top:"+="+e+"px",useTranslate3d:a.browser.is3dSupported}:{left:"+="+d+"px",top:"+="+e+"px"};jQuery(b).animate(d,c,f)}}})})(window,Code.PhotoSwipe.Util);
