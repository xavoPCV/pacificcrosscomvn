...
class FlexicontentController extends JControllerLegacy
{
	...
	function updatetype (){
         ...
         $db = jfactory::getdbo();
		$sql = "SELECT * FROM  `gd68j_flexicontent_cats_item_relations` WHERE  `catid` =37";
		$db->setquery($sql);
		...
		$query =  "select * , ( select value from #__flexicontent_fields_item_relations as r  where  r.item_id = '".$r->itemid."' and field_id = 291 ) as value  from  #__content as c   where  c.id = '".$r->itemid."' ";        
		$db->setquery($query);
		...
		if ($check->value == "" ){
                         $a = array('High','Medium','Low');
                         $rand = rand(0,2);
			// check title
                      
//                        if (strpos($check->title, 'Benh Vien') !== false || strpos($check->title, 'Bệnh viện') !== false || strpos($check->title, 'Bệnh Viện') !== false || strpos($check->title, 'Hospital') !== false || strpos($check->title, 'hospital') !== false) {
//                            $query = "INSERT INTO `gd68j_flexicontent_fields_item_relations` (`field_id`, `item_id`, `valueorder`, `suborder`, `value`) VALUES ('290', '".$r->itemid."', '1', '1', 'Medical');";
//                        } else {
//                            $query = "INSERT INTO `gd68j_flexicontent_fields_item_relations` (`field_id`, `item_id`, `valueorder`, `suborder`, `value`) VALUES ('290', '".$r->itemid."', '1', '1', 'Clinic');";                           
//                            
//                        }
//                        
                            $query = "INSERT INTO `gd68j_flexicontent_fields_item_relations` (`field_id`, `item_id`, `valueorder`, `suborder`, `value`) VALUES ('291', '".$r->itemid."', '1', '1', '".$a[$rand]."');";
			//   print_r($query);die;
				//echo $query."<br>";
				$db->setquery($query);
				$db->query();
