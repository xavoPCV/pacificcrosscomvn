...
class EnewsletterControllerWeeklyupdate extends JController {
	...
	function send(){
		...
		$query->clear()->select('*')->from('#__enewsletter')->where('`id` = '.(int)$id)->where('`email_sent_status` = 0')->where('`type` = "weeklyupdate"')->where('(`email_id` = "" OR `email_id` IS NULL)');
		$db->setQuery($query);
		...
		if ( $advisordetails && $enewsletterRow && $gid ) {
			...
			$data = array();
			$data['mdefaultemail'] = 2;//weeklyupdate.html
			...
			if ( $return && $data['email_id'] ){
				...
				$result['msg'] = JText::_('Weekly Update sent to Email Campaign system successfully.');
