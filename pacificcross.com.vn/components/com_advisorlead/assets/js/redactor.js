...
if(this.opts.air)
{$.extend(this.opts.toolbar,this.opts.buttonsCustom);this.opts.buttons=this.opts.airButtons;}
...
{var parent=$(el).parent();if(parent.size()!=0&&parent[0].tagName=='A')
{parent.remove();}
else
{$(el).remove();}
this.modalClose();this.syncCode();},imageSave:function(el)
...
{this.saveSelection();var callback=$.proxy(function()
{this.insert_link_node=false;var sel=this.getSelection();var url='',text='',target='',rel='';if(this.browser('msie'))
{var parent=this.getParentNode();if(parent.nodeName==='A')
{this.insert_link_node=$(parent);text=this.insert_link_node.text();url=this.insert_link_node.attr('href');target=this.insert_link_node.attr('target');rel=this.insert_link_node.attr('rel');}
else
{if(this.oldIE())
{text=sel.text;}
else
...
{url=sel.anchorNode.parentNode.href;text=sel.anchorNode.parentNode.text;target=sel.anchorNode.parentNode.target;rel=sel.anchorNode.parentNode.rel;if(sel.toString()==='')
...
if(rel==='nofollow')
{$('#redactor_link_nofollow').attr('checked',true);}
...
{var tab_selected=$('#redactor_tab_selected').val();var link='',text='',targettext='',target='',reltext='',rel='';if(tab_selected==='1')
...
if($('#redactor_link_nofollow').prop('checked'))
{reltext=' rel="nofollow"';rel='nofollow';}
...
this._insertLink('<a href="'+link+'"'+targettext+reltext+'>'+text+'</a>',$.trim(text),link,target,rel);},_insertLink:function(a,text,link,target,rel)
...
if(rel!=='')
{$(this.insert_link_node).attr('rel',rel);}
else
{$(this.insert_link_node).removeAttr('rel');}
this.syncCode();}
else
...
{$(this).data("textselected",false);event.type="textunselect";$.event.dispatch.apply(this,arguments)}}}}})(jQuery);