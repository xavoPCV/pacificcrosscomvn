...
$outputorder = explode(',', $this->params->get('glcorder', '0, 3, 7'));
if ($saveOrder) {
    $saveOrderingUrl = 'index.php?option=com_gnosis&task=category.saveOrderAjax&tmpl=component';
...
<form
    action="<?php echo JRoute::_('index.php?option=com_gnosis&view=category&id=' . JFactory::getApplication()->input->get('id')); ?>"
...
//$definitionstriped = substr(strip_tags($item->definition), 0, 400);
$akk = explode(" ",strip_tags($item->definition));
$definitionstriped = '';$if = 0;
foreach ($akk as $df){
	if ($if++ == 50){
	break;
	}
	$definitionstriped .= $df.' ';
...
$definitionstriped .= '...';
							...
							if ($this->params->get('glccontentplugins') == 1) {
							...
							$definitionprepare = str_replace(array('; or','; and'),'<br>',$definitionprepare);
