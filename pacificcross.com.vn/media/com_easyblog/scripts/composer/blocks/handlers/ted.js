EasyBlog.module("composer/blocks/handlers/ted", function($) {

    var module = this;

    EasyBlog.Controller("Composer.Blocks.Handlers.Ted", {
        elements: [
            '[data-ted-fieldset-{url|fluid|width-fieldset|height-fieldset|width|height|update-url}]'
        ],
        defaultOptions: $.extend({

            // Form
            "{form}": "> [data-ted-form]",
            "{insert}": "[data-ted-insert]",
            "{source}": "[data-ted-source]",
            "{loader}": "> [data-ted-loader]",

            // Error message
            "{errorMessage}": "[data-ted-error]",

            // TED player
            "{player}": "iframe",

            // Template wrapper.
            "{wrapper}": "[data-ted-wrapper]"
        ...
        return {
            ...
            isUrlValid: function(url) {
                ...
                var regex = /^(http|https):\/\/www\.ted\.com\/talks\/(.*)$/;
            ...
            crawl: function(url, block) {
                ...
                // The url needs to be made to ted's oembed service
                var url = 'http://www.ted.com/services/v1/oembed.json?url=' + encodeURIComponent(url);
