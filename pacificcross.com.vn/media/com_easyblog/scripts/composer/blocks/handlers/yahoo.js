EasyBlog.module("composer/blocks/handlers/yahoo", function($) {

    var module = this;

    EasyBlog.Controller("Composer.Blocks.Handlers.Yahoo", {
        elements: [
            '[data-yahoo-fieldset-{url|fluid|width-fieldset|height-fieldset|width|height|update-url}]'
        ],
        defaultOptions: $.extend({

            // Form
            "{form}": "> [data-yahoo-form]",
            "{insert}": "[data-yahoo-insert]",
            "{source}": "[data-yahoo-source]",
            "{loader}": "> [data-yahoo-loader]",

            // Error message
            "{errorMessage}": "[data-yahoo-error]",

            // video player
            "{player}": "iframe",

            // Template wrapper.
            "{wrapper}": "[data-yahoo-wrapper]"
        ...
        return {
            ...
            isUrlValid: function(url) {
                ...
                // https://screen.yahoo.com/popular/meghan-trainor-live-concert-140000060.html
                // https://screen.yahoo.com/meghan-trainor-live-concert-140000060.html
                var regex = /^(http|https):\/\/screen\.yahoo\.com\/(.*)$/
            ...
            crawl: function(url, block) {
                ...
                var segmentsCnt = segments.length;
                var permalink = segments[segmentsCnt - 1];

                // // lets rebuild the url
                var source = 'https://screen.yahoo.com/' + permalink + '?format=embed';
