EasyBlog.module("composer/blocks/handlers/vimeo", function($) {

    var module = this;

    EasyBlog.Controller("Composer.Blocks.Handlers.Vimeo", {
        elements: [
            '[data-vimeo-fieldset-{url|fluid|width-fieldset|height-fieldset|width|height|update-url}]'
        ],
        defaultOptions: $.extend({

            // Form
            "{form}": "> [data-vimeo-form]",
            "{insert}": "[data-vimeo-insert]",
            "{source}": "[data-vimeo-source]",
            "{loader}": "> [data-vimeo-loader]",

            // Vimeo player
            "{player}": "iframe",

            // Template wrapper.
            "{wrapper}": "[data-vimeo-wrapper]"
        ...
        return {
            ...
            crawl: function(url, block) {
                ...
                EasyBlog.ajax('site/views/crawler/crawl', {
                    ...
                    data.description = result.oembed.description;
                    data.embed = result.oembed.html;
                    data.duration = result.oembed.duration;
