...
class FD50_JSMinPlus
{
    ...
    {
        $this->parser = new FD50_JSParser($this);
    }
...
class FD50_JSParser
{
    ...
    public function parse($s, $f, $l)
    {
        ...
        $x = new FD50_JSCompilerContext(false);
        $n = $this->Script($x);
        if (!$this->t->isDone())
    ...
    {
        $n = new FD50_JSNode($this->t, JS_BLOCK);
        ...
        switch ($tt)
        {
            ...
            case KEYWORD_SWITCH:
                ...
                while (($tt = $this->t->get()) != OP_RIGHT_CURLY)
                {
                    ...
                    $n2->statements = new FD50_JSNode($this->t, JS_BLOCK);
            ...
            case TOKEN_NEWLINE:
            case OP_SEMICOLON:
                $n = new FD50_JSNode($this->t, OP_SEMICOLON);
                $n->expression = null;
            return $n;
            ...
            default:
                if ($tt == TOKEN_IDENTIFIER)
                {
                    ...
                    if ($tt == OP_COLON)
                    {
                        ...
                        $this->t->get();
                        $n = new FD50_JSNode($this->t, JS_LABEL);
                        $n->label = $label;
    ...
    private function FunctionDefinition($x, $requireName, $functionForm)
    {
        ...
        $x2 = new FD50_JSCompilerContext(true);
        ...
        while (($tt = $this->t->get()) != TOKEN_END)
        {
            ...
            switch ($tt)
            {
                ...
                case OP_HOOK:
                    ...
                    array_push($operators, new FD50_JSNode($this->t));
                ...
                case OP_DOT:
                    ...
                    {
                        $this->t->mustMatch(TOKEN_IDENTIFIER);
                        array_push($operands, new FD50_JSNode($this->t, OP_DOT, array_pop($operands), new FD50_JSNode($this->t)));
                    }
                    else
                    {
                        array_push($operators, new FD50_JSNode($this->t));
                ...
                case OP_INCREMENT: case OP_DECREMENT:
                    if ($this->t->scanOperand)
                    {
                        array_push($operators, new FD50_JSNode($this->t));  // prefix increment or decrement
                    }
                    else
                    {
                        ...
                        $n = new FD50_JSNode($this->t, $tt, array_pop($operands));
                        $n->postfix = true;
                        array_push($operands, $n);
                    }
                break;
                ...
                case TOKEN_IDENTIFIER: case TOKEN_NUMBER: case TOKEN_STRING: case TOKEN_REGEXP:
                    ...
                    array_push($operands, new FD50_JSNode($this->t));
                    $this->t->scanOperand = false;
                break;
                ...
                case OP_LEFT_BRACKET:
                    if ($this->t->scanOperand)
                    {
                        ...
                        $n = new FD50_JSNode($this->t, JS_ARRAY_INIT);
                    ...
                    else
                    {
                        ...
                        array_push($operators, new FD50_JSNode($this->t, JS_INDEX));
                        $this->t->scanOperand = true;
                        ++$x->bracketLevel;
                    }
                break;
                ...
                case OP_LEFT_CURLY:
                    ...
                    ++$x->curlyLevel;
                    $n = new FD50_JSNode($this->t, JS_OBJECT_INIT);
                    while (!$this->t->match(OP_RIGHT_CURLY))
                    {
                        do
                        {
                            ...
                            else
                            {
                                ...
                                $this->t->mustMatch(OP_COLON);
                                $n->addNode(new FD50_JSNode($this->t, JS_PROPERTY_INIT, $id, $this->Expression($x, OP_COMMA)));
                            }
                        }
                        while ($this->t->match(OP_COMMA));
                ...
                case OP_LEFT_PAREN:
                    if ($this->t->scanOperand)
                    {
                        array_push($operators, new FD50_JSNode($this->t, JS_GROUP));
                    }
                    else
                    {
                        while ( !empty($operators) &&
                        ...
                        if ($this->t->match(OP_RIGHT_PAREN))
                        {
                            ...
                            {
                                $n = new FD50_JSNode($this->t, JS_CALL, array_pop($operands), new FD50_JSNode($this->t, JS_LIST));
                            }
                        ...
                        if ($n && $n->type == KEYWORD_NEW)
                            ...
                            array_push($operators, new FD50_JSNode($this->t, JS_CALL));
                    }
                ...
                case OP_RIGHT_PAREN:
                    ...
                    if ($tt != JS_GROUP)
                    {
                        ...
                        if ($n->treeNodes[1]->type != OP_COMMA)
                            $n->treeNodes[1] = new FD50_JSNode($this->t, JS_LIST, $n->treeNodes[1]);
                        else
...
class FD50_JSTokenizer
{
    ...
    public function get($chunksize = 1000)
    {
        ...
        if (!isset($this->tokens[$this->tokenIndex]))
            $this->tokens[$this->tokenIndex] = new FD50_JSToken();
