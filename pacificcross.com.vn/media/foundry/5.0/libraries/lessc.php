...
class FD50_lessc {
	...
	protected function injectVariables($args) {
		$this->pushEnv();
		$parser = new FD50_lessc_parser($this, __METHOD__);
	...
	public function makeParser($name) {
		$parser = new FD50_lessc_parser($this, $name);
	...
	protected function newFormatter() {
		$className = "FD50_lessc_formatter_lessjs";
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = "FD50_lessc_formatter_$this->formatterName";
		}
...
class FD50_lessc_parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array('FD50_lessc', 'preg_quote'),
					array_keys(self::$precedence))).')';

			$commentSingle = FD50_lessc::preg_quote(self::$commentSingle);
			$commentMultiLeft = FD50_lessc::preg_quote(self::$commentMultiLeft);
			$commentMultiRight = FD50_lessc::preg_quote(self::$commentMultiRight);
	 ...
	 *
	 * Consider the function FD50_lessc::keyword(). (all parse functions are
	 * structured the same)
	 *
	 ...
	 *
	 * All of these parse functions are powered by FD50_lessc::match(), which behaves
	 ...
	 * avoid this, FD50_lessc::seek() is used to remember and set buffer positions.
	 *
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode("|",
			array_map(array("FD50_lessc", "preg_quote"), $directives));
	...
	protected function expressionList(&$exps) {
		...
		$exps = FD50_lessc::compressList($values, ' ');
		return true;
	}
	...
	public function propertyValue(&$value, $keyName = null) {
		...
		$value = FD50_lessc::compressList($values, ', ');
		return true;
	}
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', "@{", $end);
		$stop = array_map(array("FD50_lessc", "preg_quote"), $stop);
	...
	protected function literal($what, $eatWhitespace = null) {
		...
		if (!isset(self::$literalCache[$what])) {
			self::$literalCache[$what] = FD50_lessc::preg_quote($what);
		}
	...
	protected function to($what, &$out, $until = false, $allowNewline = false) {
		...
		if (!$this->match('('.$validChars.'*?)'.FD50_lessc::preg_quote($what), $m, !$until)) return false;
...
class FD50_lessc_formatter_classic {
	public $indentChar = "  ";
...
class FD50_lessc_formatter_compressed extends FD50_lessc_formatter_classic {
...
class FD50_lessc_formatter_lessjs extends FD50_lessc_formatter_classic {
	public $disableSingle = true;
