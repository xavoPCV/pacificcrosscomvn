...
class FD50_ClosureCompiler {
    ...
    public function min($js)
    {
        ...
        if (preg_match('/^Error\(\d\d?\):/', $response)) {
            if (is_callable($this->_fallbackFunc)) {
                ...
                throw new FD50_Minify_JS_ClosureCompiler_Exception($response);
            }
        }
        if ($response === '') {
            $errors = $this->_getResponse($this->_buildPostBody($js, true));
            throw new FD50_Minify_JS_ClosureCompiler_Exception($errors);
        }
        return $response;
    }
    ...
    protected function _getResponse($postBody)
    {
        ...
        if (false === $contents) {
            throw new FD50_Minify_JS_ClosureCompiler_Exception(
    ...
    {
        require_once(FD50_FOUNDRY_LIB . '/jsmin.php');
        return FD50_JSMinPlus::minify($js);
    }
}

class FD50_Minify_JS_ClosureCompiler_Exception extends Exception {}
