...
require_once(FD50_FOUNDRY_CLASSES . '/stylesheet/analyzer.php');
require_once(FD50_FOUNDRY_CLASSES . '/stylesheet/task.php');

class FD50_Stylesheet_Builder {
	...
	public function run($preset='cache', $options=array()) {
		...
		$this->task = new FD50_Stylesheet_Task("Building $location stylesheet '$name' using '$preset' preset.");
		$task = $this->task;
	...
	public function compileGroup($group, $options=array()) {
		...
		$task = new FD50_Stylesheet_Task("Compiling all sections for group '$group'.");
	...
	public function minifyGroup($group, $options=array()) {
		...
		$task = new FD50_Stylesheet_Task("Minifying all sections for group '$group'.");
	...
	public function buildGroup($group, $options=array()) {
		...
		$task = new FD50_Stylesheet_Task("Building group '$group'.");
	...
	public function writeTarget($group, $type, $mode) {
		...
		$task = new FD50_Stylesheet_Task("Writing $type target for '$group'.");
	...
	public function generateIndex($sections=array(), $type='css') {
		...
		$task = new FD50_Stylesheet_Task("Generating index for $type sections.");
	...
	public function joinFiles($sections=array(), $type='css') {
		...
		$task = new FD50_Stylesheet_Task("Joining $type sections.");
