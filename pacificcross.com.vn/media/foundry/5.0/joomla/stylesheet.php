...
require_once(FD50_FOUNDRY_CLASSES . '/stylesheet/compiler.php');
require_once(FD50_FOUNDRY_CLASSES . '/stylesheet/minifier.php');
require_once(FD50_FOUNDRY_CLASSES . '/stylesheet/builder.php');

jimport('joomla.filesystem.folder');
jimport('joomla.filesystem.file');
jimport('joomla.filesystem.path');

class FD50_Stylesheet {
	...
	public function file($filename, $type=null) {
		...
		if ($seek) {
			...
			$locations = FD50_Stylesheet_Compiler::importOrdering($this->location . ($this->isOverride ? '-override' : ''));
	...
	public function compiler() {
		...
		if (!isset($compiler)) {
			$compiler = new FD50_Stylesheet_Compiler($this);
		}
	...
	public function minifier() {
		...
		if (!isset($minifier)) {
			$minifier = new FD50_Stylesheet_Minifier($this);
		}
	...
	public function builder() {
		...
		if (!isset($builder)) {
			$builder = new FD50_Stylesheet_Builder($this);
		}
			...
			foreach ($filetypes as $filetype) {
				...
				if ($exists) {
					...
					if ($filetype=='css' || $filetype=='minified') {
						...
						$rules = FD50_Stylesheet_Analyzer::rules($content);
						$rules = count($rules);
					}
				} else {
					$state   = 'missing';
	...
	public function changes($fast=false) {
		...
		$task = new FD50_Foundry_Stylesheet_Task('Detect changes in stylesheets');
		...
		if (!JFile::exists($cacheFile)) {
			...
			if (!$cacheData) {
				$task->report("No cache file found at '$cacheFile'.", FD50_Foundry_Stylesheet_Task::MESSAGE_INFO);
				return $task->reject();
			}
	...
	public function purge() {
		...
		$task = new FD50_Stylesheet_Task("Purging stylesheet cache & log files.");
