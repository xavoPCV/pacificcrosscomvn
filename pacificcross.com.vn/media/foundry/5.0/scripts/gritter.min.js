...
},timer_str)},stop:function(params){var before_close=$.isFunction(params.before_close)?params.before_close:function(){},after_close=$.isFunction(params.after_close)?params.after_close:function(){},wrap=$("#gritter-notice-wrapper");before_close(wrap),wrap.fadeOut(function(){$(this).remove(),after_close()})},_str_replace:function(search,replace,subject,count){var i=0,j=0,temp="",repl="",sl=0,fl=0,f=[].concat(search),r=[].concat(replace),s=subject,ra=r instanceof Array,sa=s instanceof Array;for(s=[].concat(s),count&&(this.window[count]=0),i=0,sl=s.length;sl>i;i++)if(""!==s[i])for(j=0,fl=f.length;fl>j;j++)temp=s[i]+"",repl=ra?void 0!==r[j]?r[j]:"":r[0],s[i]=temp.split(f[j]).join(repl),count&&s[i]!==temp&&(this.window[count]+=(temp.length-s[i].length)/f[j].length);
return sa?s:s[0]},_verifyWrapper:function(){$("body > [id=fd].gritter").length<1&&$(document.createElement("div")).attr("id","fd").addClass("gritter").css({position:"absolute",top:0,left:0,overflow:"visible",width:0,height:0,zIndex:10003}).appendTo("body"),0==$("#gritter-notice-wrapper").length&&$("[id=fd].gritter").append(this._tpl_wrap)}}}(jQuery)};exports(),module.resolveWith(exports)};FD50.module("gritter",moduleFactory)}();