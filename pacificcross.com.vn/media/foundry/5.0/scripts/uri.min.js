FD50.plugin("uri",function($){$.isUrl=function(s){var regexp=/^(http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;return regexp.test(s)};var Query=function(queryString){"use strict";var parseQuery=function(q){var i,ps,p,kvp,k,v,arr=[];if("undefined"==typeof q||null===q||""===q)return arr;for(0===q.indexOf("?")&&(q=q.substring(1)),ps=q.toString().split(/[&;]/),i=0;i<ps.length;i++)p=ps[i],kvp=p.split("="),k=kvp[0],v=-1===p.indexOf("=")?null:null===kvp[1]?"":kvp[1],arr.push([k,v]);return arr
