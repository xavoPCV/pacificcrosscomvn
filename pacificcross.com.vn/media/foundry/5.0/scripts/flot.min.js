...
}function fillInSeriesOptions(){var i,neededColors=series.length,maxIndex=-1;for(i=0;i<series.length;++i){var sc=series[i].color;null!=sc&&(neededColors--,"number"==typeof sc&&sc>maxIndex&&(maxIndex=sc))}maxIndex>=neededColors&&(neededColors=maxIndex+1);var c,colors=[],colorPool=options.colors,colorPoolSize=colorPool.length,variation=0;for(i=0;neededColors>i;i++)c=$.color.parse(colorPool[i%colorPoolSize]||"#666"),i%colorPoolSize==0&&i&&(variation=variation>=0?.5>variation?-variation-.2:0:-variation),colors[i]=c.scale("rgb",1+variation);
...
var info=this.getTextInfo(layer,text,font,angle,width),positions=info.positions,lines=info.lines;y+=info.height/lines.length/2,y=Math.round("middle"==valign?y-info.height/2:"bottom"==valign?y-info.height:y),window.opera&&window.opera.version().split(".")[0]<12&&(y-=2);for(var position,i=0;position=positions[i];i++)if(position.x==x&&position.y==y)return void(position.active=!0);position={active:!0,lines:[],x:x,y:y},positions.push(position);for(var line,i=0;line=lines[i];i++)position.lines.push("center"==halign?[Math.round(x-line.width/2),y]:"right"==halign?[Math.round(x-line.width),y]:[Math.round(x),y]),y+=line.height
