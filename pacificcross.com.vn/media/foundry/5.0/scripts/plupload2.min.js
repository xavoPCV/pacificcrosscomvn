...
}),this.files=null}});var trigger=self.trigger;self.trigger=function(eventName){var result=trigger.apply(self,arguments);return $(options.browse_button).trigger("fileinput"+eventName,[self].concat($.makeArray(arguments).slice(1))),result}}var dispatches=["ready","change","cancel","mouseenter","mouseleave","mousedown","mouseup"];return FileInput.prototype=EventTarget.instance,FileInput}),define("moxie/file/FileDrop",["moxie/core/I18n","moxie/core/utils/Dom","moxie/core/Exceptions","moxie/core/utils/Basic","moxie/file/File","moxie/runtime/RuntimeClient","moxie/core/EventTarget","moxie/core/utils/Mime"],function(I18n,Dom,x,Basic,File,RuntimeClient,EventTarget,Mime){function FileDrop(options){var defaults,self=this;
"string"==typeof options&&(options={drop_zone:options}),defaults={accept:[{title:I18n.translate("All Files"),extensions:"*"}],required_caps:{drag_and_drop:!0},multiple:!0},options="object"==typeof options?Basic.extend({},defaults,options):defaults,options.container=Dom.get(options.drop_zone)||document.body,"static"===Dom.getStyle(options.container,"position")&&(options.container.style.position="relative"),"string"==typeof options.accept&&(options.accept=Mime.mimes2extList(options.accept)),RuntimeClient.call(self),Basic.extend(self,{uid:Basic.guid("uid_"),ruid:null,files:null,init:function(){self.convertEventPropsToHandlers(dispatches),self.bind("RuntimeInit",function(e,runtime){self.ruid=runtime.uid,self.bind("Drop",function(){var files=runtime.exec.call(self,"FileDrop","getFiles");
self.files=[],options.multiple||(files=files[0]),Basic.each(files,function(file){self.files.push(new File(self.ruid,file))})},999),runtime.exec.call(self,"FileDrop","init",options),self.dispatchEvent("ready")}),self.connectRuntime(options)},destroy:function(){var runtime=this.getRuntime();runtime&&(runtime.exec.call(this,"FileDrop","destroy"),this.disconnectRuntime()),this.files=null}});var trigger=self.trigger;self.trigger=function(eventName){var result=trigger.apply(self,arguments);return $(options.drop_zone).trigger("filedrop"+eventName,[self].concat($.makeArray(arguments).slice(1))),result
}}var dispatches=["ready","dragenter","dragleave","drop","error"];return FileDrop.prototype=EventTarget.instance,FileDrop}),define("moxie/runtime/RuntimeTarget",["moxie/core/utils/Basic","moxie/runtime/RuntimeClient","moxie/core/EventTarget"],function(Basic,RuntimeClient,EventTarget){function RuntimeTarget(){this.uid=Basic.guid("uid_"),RuntimeClient.call(this),this.destroy=function(){this.disconnectRuntime(),this.unbindAll()}}return RuntimeTarget.prototype=EventTarget.instance,RuntimeTarget}),define("moxie/file/FileReader",["moxie/core/utils/Basic","moxie/core/utils/Encode","moxie/core/Exceptions","moxie/core/EventTarget","moxie/file/Blob","moxie/file/File","moxie/runtime/RuntimeTarget"],function(Basic,Encode,x,EventTarget,Blob,File,RuntimeTarget){function FileReader(){function _read(op,blob){function error(err){self.readyState=FileReader.DONE,self.error=err,self.trigger("error"),loadEnd()
}function loadEnd(){_fr.destroy(),_fr=null,self.trigger("loadend")}function exec(runtime){_fr.bind("Error",function(e,err){error(err)}),_fr.bind("Progress",function(e){self.result=runtime.exec.call(_fr,"FileReader","getResult"),self.trigger(e)}),_fr.bind("Load",function(e){self.readyState=FileReader.DONE,self.result=runtime.exec.call(_fr,"FileReader","getResult"),self.trigger(e),loadEnd()}),runtime.exec.call(_fr,"FileReader","read",op,blob)}if(_fr=new RuntimeTarget,this.convertEventPropsToHandlers(dispatches),this.readyState===FileReader.LOADING)return error(new x.DOMException(x.DOMException.INVALID_STATE_ERR));
...
})),caps}function processInlineOptions(options){var inlineOptions={};return $.each(options,function(prop,val){prop=prop.replace(/-/g,"_"),/filters|headers|multipart_params|resize|mime_types/.test(prop)&&(val=JSON.parse(val)),/multipart|prevent_duplicates|unique_names|multi_selection/.test(prop)&&(val=/true|1/i.test(val),val=val?"1":"0"),/max_retries/.test(prop)&&(val=parseInt(val)),/extensions/.test(prop)||(/prevent_duplicates|max_file_size/.test(prop)?(inlineOptions.filters||(inlineOptions.filters={}))[prop]=val:inlineOptions[prop]=val)
}),inlineOptions}var delay=window.setTimeout,fileFilters={},plupload={VERSION:"2.1.2",STOPPED:1,STARTED:2,QUEUED:1,UPLOADING:2,FAILED:4,DONE:5,GENERIC_ERROR:-100,HTTP_ERROR:-200,IO_ERROR:-300,SECURITY_ERROR:-400,INIT_ERROR:-500,FILE_SIZE_ERROR:-600,FILE_EXTENSION_ERROR:-601,FILE_DUPLICATE_ERROR:-602,IMAGE_FORMAT_ERROR:-700,MEMORY_ERROR:-701,IMAGE_DIMENSIONS_ERROR:-702,mimeTypes:o.mimes,ua:o.ua,typeOf:o.typeOf,extend:o.extend,guid:o.guid,get:function(ids){var el,els=[];"array"!==o.typeOf(ids)&&(ids=[ids]);for(var i=ids.length;i--;)el=o.get(ids[i]),el&&els.push(el);
...
}function initControls(settings,cb){var self=this,inited=0,queue=[],options={runtime_order:settings.runtimes,required_caps:settings.required_features,preferred_caps:preferred_caps,swf_url:$.path+"/scripts/plupload2/Moxie.swf",xap_url:$.path+"/scripts/plupload2/Moxie.xap"};plupload.each(settings.runtimes.split(/\s*,\s*/),function(runtime){settings[runtime]&&(options[runtime]=settings[runtime])}),settings.browse_button&&plupload.each(settings.browse_button,function(el){queue.push(function(cb){var fileInput=addFileInput.call(self,el,plupload.extend(options,{autoinit:!1})),onready=fileInput.onready;
fileInput.onready=function(){onready.apply(fileInput,arguments),inited++,cb()},fileInput.bind("error runtimeerror",function(){cb()}),fileInput.init()})}),settings.drop_element&&plupload.each(settings.drop_element,function(el){queue.push(function(cb){var fileDrop=addFileDrop.call(self,el,plupload.extend(options,{autoinit:!1})),onready=fileDrop.onready;fileDrop.onready=function(){onready.apply(fileDrop,arguments),inited++,cb()},fileDrop.bind("error runtimeerror",function(){cb()}),fileDrop.init()})}),o.inSeries(queue,function(){"function"==typeof cb&&cb(inited)
})}function addFileInput(el,options){var self=this,settings=this.settings,defaultOptions={accept:settings.filters.mime_types,name:settings.file_data_name,multiple:settings.multi_selection,container:settings.container,browse_button:el,autoinit:!0},options=plupload.extend(defaultOptions,options),fileInput=new o.FileInput(options);return fileInput.onready=function(){var info=o.Runtime.getInfo(this.ruid);o.extend(self.features,{chunks:info.can("slice_blob"),multipart:info.can("send_multipart"),multi_selection:info.can("select_multiple")}),fileInputs.push(this)
},fileInput.onchange=function(){self.addFile(this.files,null,this)},fileInput.bind("mouseenter mouseleave mousedown mouseup",function(e){disabled||(settings.browse_button_hover&&("mouseenter"===e.type?o.addClass(el,settings.browse_button_hover):"mouseleave"===e.type&&o.removeClass(el,settings.browse_button_hover)),settings.browse_button_active&&("mousedown"===e.type?o.addClass(el,settings.browse_button_active):"mouseup"===e.type&&o.removeClass(el,settings.browse_button_active)))}),fileInput.bind("mousedown",function(){self.trigger("Browse")
}),fileInput.bind("error runtimeerror",function(){fileInput=null}),options.autoinit&&fileInput.init(),fileInput}function addFileDrop(el,options){var self=this,settings=this.settings,options=plupload.extend({autoinit:!0},options,{drop_zone:el,multiple:settings.multi_selection}),fileDrop=new o.FileDrop(options);return fileDrop.onready=function(){var info=o.Runtime.getInfo(this.ruid);self.features.dragdrop=info.can("drag_and_drop"),fileDrops.push(this)},fileDrop.ondrop=function(){self.addFile(this.files,null,this)
},fileDrop.bind("error runtimeerror",function(){fileDrop=null}),options.autoinit&&fileDrop.init(),fileDrop}function resizeImage(blob,params,cb){var img=new o.Image;try{img.onload=function(){return params.width>this.width&&params.height>this.height&&params.quality===undef&&params.preserve_headers&&!params.crop?(this.destroy(),cb(blob)):void img.downsize(params.width,params.height,params.crop,params.preserve_headers)},img.onresize=function(){cb(this.getAsBlob(blob.type,params.quality)),this.destroy()},img.onerror=function(){cb(blob)
},img.load(blob)}catch(ex){cb(blob)}}function setOption(option,value,init){function _setOption(option,value,init){var oldValue=settings[option];switch(option){case"max_file_size":"max_file_size"===option&&(settings.max_file_size=settings.filters.max_file_size=value);break;case"chunk_size":(value=plupload.parseSize(value))&&(settings[option]=value,settings.send_file_name=!0);break;case"multipart":settings[option]=value,value||(settings.send_file_name=!0);break;case"unique_names":settings[option]=value,value&&(settings.send_file_name=!0);
...
},1)))}function onDestroy(up){up.stop(),plupload.each(files,function(file){file.destroy()}),files=[],fileInputs.length&&(plupload.each(fileInputs,function(fileInput){fileInput.destroy()}),fileInputs=[]),fileDrops.length&&(plupload.each(fileDrops,function(fileDrop){fileDrop.destroy()}),fileDrops=[]),preferred_caps={},disabled=!1,startTime=xhr=null,total.reset()}var settings,startTime,total,xhr,uid=plupload.guid(),files=[],preferred_caps={},fileInputs=[],fileDrops=[],disabled=!1;settings={runtimes:o.Runtime.order,max_retries:0,chunk_size:0,multipart:!0,multi_selection:!0,file_data_name:"file",flash_swf_url:$.path+"/scripts/plupload2/js/Moxie.swf",silverlight_xap_url:$.path+"/scripts/plupload2/js/Moxie.xap",filters:{mime_types:[],prevent_duplicates:!1,max_file_size:0},resize:{enabled:!1,preserve_headers:!0,crop:!1},send_file_name:!0,send_chunk_number:!0},setOption.call(this,options,null,!0),total=new plupload.QueueProgress,plupload.extend(this,{id:uid,uid:uid,state:plupload.STOPPED,features:{},runtime:null,files:files,settings:settings,total:total,init:function(){var self=this,trigger=self.trigger;
return self.trigger=function(eventName){var result=trigger.apply(this,arguments);return $(settings.container||settings.browse_button).trigger("plupload"+eventName,[self].concat($.makeArray(arguments).slice(1))),result},"function"==typeof settings.preinit?settings.preinit(self):plupload.each(settings.preinit,function(func,name){self.bind(name,func)}),bindEventListeners.call(this),settings.browse_button&&settings.url?void initControls.call(this,settings,function(inited){"function"==typeof settings.init?settings.init(self):plupload.each(settings.init,function(func,name){self.bind(name,func)
}),inited?(self.runtime=o.Runtime.getInfo(getRUID()).type,self.trigger("Init",{runtime:self.runtime}),self.trigger("PostInit")):self.trigger("Error",{code:plupload.INIT_ERROR,message:plupload.translate("Init error.")})}):void this.trigger("Error",{code:plupload.INIT_ERROR,message:plupload.translate("Init error.")})},setOption:function(option,value){setOption.call(this,option,value,!this.runtime)},getOption:function(option){return option?settings[option]:settings},refresh:function(){fileInputs.length&&plupload.each(fileInputs,function(fileInput){fileInput.trigger("Refresh")
}),this.trigger("Refresh")},start:function(){this.state!=plupload.STARTED&&(this.state=plupload.STARTED,this.trigger("StateChanged"),uploadNext.call(this))},stop:function(){this.state!=plupload.STOPPED&&(this.state=plupload.STOPPED,this.trigger("StateChanged"),this.trigger("CancelUpload"))},disableBrowse:function(){disabled=arguments[0]!==undef?arguments[0]:!0,fileInputs.length&&plupload.each(fileInputs,function(fileInput){fileInput.disable(disabled)}),this.trigger("DisableBrowse",disabled)},getFile:function(id){var i;
for(i=files.length-1;i>=0;i--)if(files[i].id===id)return files[i]},addFile:function(file,fileName,origin){function filterFile(file,cb){var queue=[];o.each(self.settings.filters,function(rule,name){fileFilters[name]&&queue.push(function(cb){fileFilters[name].call(self,rule,file,function(res){cb(!res)})})}),o.inSeries(queue,cb)}function resolveFile(file){var type=o.typeOf(file);if(file instanceof o.File){if(!file.ruid&&!file.isDetached()){if(!ruid)return!1;file.ruid=ruid,file.connectRuntime(ruid)}resolveFile(new plupload.File(file))
}else file instanceof o.Blob?(resolveFile(file.getSource()),file.destroy()):file instanceof plupload.File?(fileName&&(file.name=fileName),file.origin=origin,queue.push(function(cb){filterFile(file,function(err){err||(files.push(file),filesAdded.push(file),self.trigger("FileFiltered",file)),delay(cb,1)})})):-1!==o.inArray(type,["file","blob"])?resolveFile(new o.File(null,file)):"node"===type&&"filelist"===o.typeOf(file.files)?o.each(file.files,resolveFile):"array"===type&&(fileName=null,o.each(file,resolveFile))
}var ruid,self=this,queue=[],filesAdded=[];options&&delete options.filename,ruid=getRUID(),resolveFile(file),queue.length&&o.inSeries(queue,function(){filesAdded.length&&self.trigger("FilesAdded",filesAdded)})},removeFile:function(file){for(var id="string"==typeof file?file:file.id,i=files.length-1;i>=0;i--)if(files[i].id===id)return this.splice(i,1)[0]},splice:function(start,length){var removed=files.splice(start===undef?0:start,length===undef?files.length:length),restartRequired=!1;return this.state==plupload.STARTED&&(plupload.each(removed,function(file){return file.status===plupload.UPLOADING?(restartRequired=!0,!1):void 0
}),restartRequired&&this.stop()),this.trigger("FilesRemoved",removed),plupload.each(removed,function(file){file.destroy()}),restartRequired&&this.start(),removed},bind:function(name,func,scope){var self=this;plupload.Uploader.prototype.bind.call(this,name,function(){var args=[].slice.call(arguments);return args.splice(0,1,self),func.apply(this,args)},0,scope)},addFileInput:function(){return addFileInput.apply(this,arguments)},addFileDrop:function(){return addFileDrop.apply(this,arguments)},destroy:function(){this.trigger("Destroy"),settings=total=null,this.unbindAll()
}})},plupload.Uploader.prototype=o.EventTarget.instance,plupload.File=function(){function PluploadFile(file){plupload.extend(this,{id:plupload.guid(),name:file.name||file.fileName,type:file.type||"",size:file.size||file.fileSize,origSize:file.size||file.fileSize,loaded:0,percent:0,status:plupload.QUEUED,lastModifiedDate:file.lastModifiedDate||(new Date).toLocaleString(),getNative:function(){var file=this.getSource().getSource();return-1!==o.inArray(o.typeOf(file),["blob","file"])?file:null},getSource:function(){return filepool[this.id]?filepool[this.id]:null
},destroy:function(){var src=this.getSource();src&&(src.destroy(),delete filepool[this.id])}}),filepool[this.id]=file}var filepool={};return PluploadFile}(),plupload.QueueProgress=function(){var self=this;self.size=0,self.loaded=0,self.uploaded=0,self.failed=0,self.queued=0,self.percent=0,self.bytesPerSec=0,self.reset=function(){self.size=self.loaded=self.uploaded=self.failed=self.queued=self.percent=self.bytesPerSec=0}},$.plupload2=plupload,$.fn.plupload2=function(options,autoinit){var container=$(this[0]);
if(!container.data("uploader")){var autoinit=void 0===autoinit?!0:autoinit,defaultOptions={runtimes:"html5, flash, html4",browse_button:container.find("[data-plupload-browse-button]")[0],drop_element:container.find("[data-plupload-drop-element]")[0]},inlineOptions=processInlineOptions(container.htmlData("plupload",!1)),options=$.extend(defaultOptions,inlineOptions,options),extensions=options.extensions;extensions&&((options.filters||(options.filters={})).mime_types=[{title:"Accepted files",extensions:extensions}]);
var uploader=new $.plupload2.Uploader(options);return container.data("uploader",uploader),container.on("mouseover","[data-plupload-browse-button]",function(){var button=$(this);if(!button.data("fileinput")){var options={container:button.data("data-fileinput-container")||button.parent()[0]};uploader.addFileInput(button[0],options)}}),uploader.bind("Destroy",function(){container.removeData("uploader")}),autoinit&&uploader.init(),uploader}}}(window,$.moxie)};exports(),module.resolveWith(exports)};FD50.module("plupload2",moduleFactory)
}();