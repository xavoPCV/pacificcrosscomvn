...
})}var win=$(window),repositionOnResize="resize.popbox"+popbox.uid;if(win.off(repositionOnResize).on(repositionOnResize,function(){popbox.tooltip.length>0&&popbox.tooltip.position(popbox.position)}),popbox.tooltip.length>0)return popbox.tooltip.appendTo("body").position(popbox.position),void popbox.trigger("popboxActivate",[popbox]);if($.isFunction(popbox.content)){var options=popbox.content(popbox);if(popbox.update(options),popbox.wait)return}$.isDeferred(popbox.content)&&("pending"==popbox.content.state()&&(popbox.loader.appendTo("body").position(popbox.position),popbox.trigger("popboxLoading",[popbox])),popbox.content.always(function(){popbox.wait=!1
