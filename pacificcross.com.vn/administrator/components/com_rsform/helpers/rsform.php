...
class RSFormProHelper
{
	...
	public static function componentExists($formId, $componentTypeId)
	{
		...
		if (is_array($componentTypeId))
		{
			...
			$db->setQuery("SELECT ComponentId FROM #__rsform_components WHERE ComponentTypeId IN (".implode(',', $componentTypeId).") AND FormId='".$formId."' AND Published='1' order by ComponentId ");
		}
		else
		{
			$componentTypeId = (int) $componentTypeId;
			$db->setQuery("SELECT ComponentId FROM #__rsform_components WHERE ComponentTypeId='".$componentTypeId."' AND FormId='".$formId."' AND Published='1'  order by ComponentId ");
		}
	...
	public static function showForm($formId, $val=array(), $validation=array())
	{
		...
		if (!empty($calendars))
		{
			...
			foreach ($calendars as $i => $calendarComponentId)
			{
				...
				$calendars['VALUES'][$i] = $data['MINDATE'];
                                                                                       $futureDate = date('m/d/Y', strtotime(' +1 day')) ;
                                $agefrom = date('m/d/Y', strtotime("-18 year") ) ;
                                $weekage= date('m/d/Y', strtotime("-6 week") ) ; 
                                
                                if ($calendars['VALUES'][$i] == '01/01/2013' ) {
                                    $calendars['VALUES'][$i] = $futureDate;
                                }
if ($calendars['VALUES'][$i] == '01/01/2012' ) {
                                    $calendars['VALUES'][$i] = $futureDate;
                                }
                                
                                if ($calendars['VALUES'][$i] == '02/02/2012' ) {
                                    $calendars['VALUES'][$i] = $agefrom;
                                }
if ($data['MAXDATE'] == '01/01/2016' ) {
                                    $calendars['VALUES'][$i] = $weekage;
                                }
