...
class CPDF_Adapter implements Canvas {
  ...
  protected function _add_page_text() {
    ...
    if(isset($post['form']['iscover'][0])){
            if ($post['form']['iscover'][0] == 'Yes'){
                $pagebread = 1;
            }
    }elseif(isset($hash)){
    ...
    if (strlen($hash) == 32)
			{
				$type = $task = 'user';
				$db = JFactory::getDBO();
				$db->setQuery("SELECT * FROM #__rsform_submissions WHERE MD5(CONCAT(SubmissionId, '{".$type."}', DateSubmitted)) = '".$db->escape($hash)."'");
				 
                                $row = $db->loadObject();
                                
                                $db = JFactory::getDBO();
                                $db->setQuery("SELECT FieldValue FROM #__rsform_submission_values WHERE FormId = '".$row->FormId."' and SubmissionId =  '".$row->SubmissionId."' and FieldName = 'iscover' ");
                                
                                $row1 = $db->loadObject();
                                if (isset($row1->FieldValue)){
                                    if( $row1->FieldValue == 'Yes' ) {
                                        $pagebread = 1;
                                    }
                                }
			}
