...
if ($this->_form->ShowFormTitle) {
	$out.='<h1 class="page-title">{global:formtitle}</h1>'."\n";
}
...
{	
	...
	$out.="\t".'<div class="rsform-block rsform-block-'.JFilterOutput::stringURLSafe($quickfield).'">'."\n";
	$out.= "\t\t".'<div class="caption">{'.$quickfield.':caption}'.$required.'</div>'."\n";
	$out.= "\t\t".'<div class="body">{'.$quickfield.':body}<span class="formClr">{'.$quickfield.':validation}</span></div>'."\n";
	$out.= "\t\t".'<div class="description">{'.$quickfield.':description}</div>'."\n";
	$out.="\t".'</div>'."\n";
}
