...
{	
	...
	function aabs()
	{
		...
		$query->select('*')->from('#__rsform_properties')->where( " `PropertyValue` LIKE  'Option3Person4Original%'  and  PropertyName = 'NAME' " );
		$db->setquery($query);
		...
		foreach ($data as $r) {
		      ...
		      $a =	count( explode('copy', $r->PropertyValue ) );
			 $a =   $a-1;
			 if ( $a > 0) {
			 
//			
//				 echo  $r->ComponentId.' : '.$a.' <br>';
//                                 
				 $aaat =  str_replace('4', ($a+4) , str_replace(' copy', '' , $r->PropertyValue));
				// echo  $aaat.'<br>';
//                 $query = $db->getquery(true);
//                 $query->update('#__rsform_properties')->set("PropertyValue = '".$aaat."'")->where( " ComponentId = '$r->ComponentId' and PropertyId = '".$r->PropertyId."' " );$db->setquery($query);$db->query();
//                                 
//                                 
//                                 
//                                $query = $db->getquery(true);
//                                $query->select('*')->from('#__rsform_properties')->where( "   PropertyName = 'Caption'  and ComponentId =  '$r->ComponentId' " );
//                                $db->setquery($query);
//                                $data2 = $db->loadobject();
//                                
//                             $aaab =  str_replace('9th', $a+9 ,  $data2->PropertyValue );
//							//echo $aaab."<br>";	
//								
//                                $query = $db->getquery(true);
//                                $query->update('#__rsform_properties')->set("PropertyValue = '".$aaab."'")->where( " ComponentId = '$data2->ComponentId' and PropertyId = '".$data2->PropertyId."' " );$db->setquery($query);$db->query();
//                             
