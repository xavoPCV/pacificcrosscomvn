...
class FaqModelUpdates extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = array(
                ...
                'update_category_id', 'a.update_category_id',
                'update_title', 'a.update_title',
    ...
    protected function populateState($ordering = null, $direction = null) {
        ...
        parent::populateState('a.update_date', 'asc');
    }
    ...
    protected function getListQuery() {
		...
		$query->select('faq_category_id.title AS update_category_name');
    ...
    if (is_numeric($published)) {
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.update_title LIKE '.$search.' )');
            }
        }
